...
- 原生 macOS App 改为 Universal 2 构建，同时支持 Apple Silicon 与 Intel Mac。
- GitHub 首页重做首屏信息：增加直接下载入口、应用截图、功能亮点、安装步骤与动态版本徽章。
- 完善仓库简介、主页地址与主题标签，方便对外分享和检索。
- 新增产品落地页，以及随 Skill 打包的产品配置与普通用户使用指南。

## 1.4.2 — 2026-07-19

### 新增

- 主题库工具栏和菜单栏快速切换区新增“刷新皮肤”按钮。
- 刷新时会重新同步内置预设并扫描本机主题库，完成后显示新增数量与主题总数。

### 改进

- “刷新运行状态”与“刷新皮肤数据”拆分，避免运行状态轮询造成主题列表长期停留在应用启动时的快照。
- 工作台品牌标题统一为 `CODEX THEME STUDIO`。
...
- 原生主题工作台加入搜索、分类筛选、当前主题大图预览与更清晰的自动化控制面板。
- 新增「野生博物」「钴蓝工坊」「朱砂信号」「瓷白纸页」四套程序化纯色主题；「赛博霓虹」同步纳入纯色目录。
- 新增主题目录命令，支持列出、搜索、安装、导入和程序化生成纯色主题，并在原子安装前执行完整 payload 校验。
- 新增 `create-codex-dream-skin` Skill：可从自然语言要求搜索现有皮肤，或生成图片/纯色主题后导入安装。

### 改进

- SwiftUI 界面重构为深色编辑室视觉，补充悬停反馈、空搜索状态、VoiceOver 标签和减少动态效果适配。
- 登录项与 Codex 运行监听从主控制器拆分为独立系统服务，移除单个皮肤的硬编码排序。
- 状态检测改用完整进程命令匹配，兼容 macOS 对 `ChatGPT` 进程名的截断行为。
- 发布流程新增可直接下载的 macOS App ZIP、Skill ZIP、校验和与 GitHub Release 文档。

## 1.3.0 — 2026-07-19

### 新增

- 新增原生 SwiftUI 菜单栏应用：统一管理皮肤开关、主题预览与切换、背景图导入、状态修复和日志入口。
- 新增基于 `SMAppService` 的登录启动开关，以及 Codex 普通启动后的单次自动接管。
- 新增串行化管理命令与跨进程锁；开启和关闭只允许一个生命周期操作，避免重复注入与重启竞争。

### 改进

- 主题切换在活动会话中走热更新，关闭状态只暂存选择，不会无故启动 Codex。
- 登录接管加入 90 秒自动重试熔断，并始终尊重用户主动退出 Codex。
- 首次启动原生管理器时迁移旧的一次性 LaunchAgent 与同名 SwiftBar 自动入口。
