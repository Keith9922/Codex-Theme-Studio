...
APP_ARCHIVE="$RELEASE_ROOT/Codex-Theme-Studio-macOS-v$VERSION.zip"
SKILL_ARCHIVE="$RELEASE_ROOT/create-codex-theme-skill-v$VERSION.zip"
...
COPYFILE_DISABLE=1 /usr/bin/ditto -c -k --sequesterRsrc --keepParent \
  "$RELEASE_ROOT/Codex Theme Studio.app" "$APP_ARCHIVE"
COPYFILE_DISABLE=1 /usr/bin/ditto -c -k --keepParent --norsrc --noextattr \
  "$REPO_ROOT/skills/create-codex-theme" "$SKILL_ARCHIVE"
